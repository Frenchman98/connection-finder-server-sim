...
void process(char *_result) {
    // Make a copy first
    char *result = calloc(strlen(_result) + 5, sizeof(char));
    strcpy(result, _result);
    char *tok = strtok(result, ";"), 
         *endptr, *name_a, *name_b;
    ...
    char *path = calloc(2, sizeof(char)), *tmp, *ptok;
...
int main(int argc, char *argv[]) {
    ...
    addr.sin_port = htons(central_tcp_b_port);
    ...
    if (send(fd, msg, strlen(msg), 0) < 0) {
        fprintf(stderr, "There was a problem sending the entered name(s) to the central server.\n");
        return 1;
    }

    // Send update to terminal
    if (argc == 2) printf("The client sent %s to the Central server.\n", msg);
    else printf("The client sent %s and %s to the Central server.\n", argv[1], argv[2]);
